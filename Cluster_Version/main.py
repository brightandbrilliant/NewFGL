...
from Cluster import load_pyg_data, load_anchors, compute_anchor_feature_differences, assign_weights_by_threshold
...
if __name__ == "__main__":
    ...
    data1, data2 = load_pyg_data(pyg_data_files[0]), load_pyg_data(pyg_data_files[1])
    anchor_pairs = load_anchors(anchor_path)
    anchor_diffs = compute_anchor_feature_differences(data1, data2, anchor_pairs)
    weighted_anchors = assign_weights_by_threshold(anchor_diffs, thresholds=[0.1, 0.35], weights=[1.0, 0.5, 0.1])

    clients = load_all_clients(pyg_data_files, weighted_anchors, encoder_params,
    ...
    for rnd in range(1, num_rounds + 1):
        ...
        metrics, _ = evaluate_all_clients(clients, False)
