...
def load_all_clients(pyg_data_paths, encoder_params, decoder_params, training_params, device, mu=0.01):
    clients = []
...
if __name__ == "__main__":
    ...
    num_rounds = 600
    mu = 0.01  # FedProx 的近端正则系数
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

    # 2. 初始化客户端
    clients = load_all_clients(pyg_data_files, encoder_params, decoder_params, training_params, device, mu=mu)

    best_f1 = -1
    best_encoder_state = None
    best_decoder_state = None

    print("\n================ Federated Training Start (FedProx) ================\n")
    for rnd in range(1, num_rounds + 1):
        ...
        if rnd == 1:
            # 初始化时全局参数取第一个 client
            global_encoder_state = clients[0].get_encoder_state()
            global_decoder_state = clients[0].get_decoder_state()
