...
from ProxProClient import Client as ProxClient
...
def load_all_clients(pyg_data_paths, encoder_params, decoder_params, training_params, device, nClusters=10, mu=0.01):
...
if __name__ == "__main__":
    ...
    clients, cluster_labels, raw_data_list, edge_dicts = load_all_clients(
        pyg_data_files, encoder_params, decoder_params, training_params, device, nClusters, mu=prox_mu
    )

    # compute initial global states by averaging initial local states
    ...
    # set initial global model and set global_state for prox
    for client in clients:
        client.set_encoder_state(global_encoder_state)
        client.set_decoder_state(global_decoder_state)
        client.set_global_state(global_encoder_state, global_decoder_state)
    ...
    # sliding windows and bookkeeping
    ...
    loss_record = [[] for _ in range(len(clients))]
    ...
    for rnd in range(1, num_rounds + 1):
        ...
        # 1) 下发全局模型并设置 prox 全局 state（保证 prox 使用最新 global）
        for client in clients:
            client.set_encoder_state(global_encoder_state)
            client.set_decoder_state(global_decoder_state)
            client.set_global_state(global_encoder_state, global_decoder_state)

        # 2) 使用下发后的全局 encoder 计算跨客户端 embeddings（用于 augment 注入）
        #    这样保证 injected embeddings 来自统一的 global 表示
        ...
        for i, client in enumerate(clients):
            ...
            # 在增强触发轮次，执行增强训练（注意调用位置与你的原 main 保持一致）
            if augment_flag[i] and rnd % enhance_interval == 0:
        ...
        # 5) 收集本地模型并聚合为新的全局模型（encoder & decoder 都聚合）
        encoder_states = [client.get_encoder_state() for client in clients]
        decoder_states = [client.get_decoder_state() for client in clients]
        global_encoder_state = average_state_dicts(encoder_states)
        global_decoder_state = average_state_dicts(decoder_states)

        # 6) 将新的 global 直接下发给各客户端以便下一轮使用（这里也用于评估）
        for client in clients:
            client.set_encoder_state(global_encoder_state)
            client.set_decoder_state(global_decoder_state)

        # 7) 验证评估（使用每个 client 的 validation 边）
    ...
    # 最终：将 best global encoder 下发给所有 client，并对 decoder 使用各自 best decoder
